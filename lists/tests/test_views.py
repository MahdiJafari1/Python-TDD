...
class ListViewTest(TestCase):
    ...
    def test_validation_errors_end_up_on_lists_page(self):
        list_ = List.objects.create()
        response = self.client.post(
            reverse("view_list", args=[list_.id]), data={"text": ""}
        )
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "list.html")
        self.assertContains(response, "You cannot have an empty list item")
    ...
    def post_invalid_input(self):
        list_ = List.objects.create()
        return self.client.post(reverse("view_list", args=[list_.id]),data={"text": ""})
    ...
    def test_for_invalid_input_passess_form_to_template(self):
        response = self.post_invalid_input()
        self.assertIsInstance(response.context["form"], ItemForm)
